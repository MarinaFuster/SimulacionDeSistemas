...
public class OscillatorParticle {
    ...
    private void applyGearPC(double deltaT, double elasticConstant, double damping) {
        ...
        double deltaA = Math.abs(r2tdeltaT - r2ptdeltaT); // TODO: check if this should be abs

        // correction
        double deltaR2 = deltaA * deltaT * deltaT / 2;

        // alphas to correct predicted values
        double c[] = {3/16, 251/360, 1, 11/18, 1/6, 1/60};

        // updates x and v
        x = RPredictedVector[0] + c[0]*deltaR2;
        v = RPredictedVector[1] + c[1]*deltaR2;

        // apply corrections and save them
        for(int i=0; i<6; i++) {
            previousRVector[i] = RPredictedVector[i] + c[i]*deltaR2;
        }
    }
    ...
    private double[][] multiplier(double deltaT) {
        double matrix[][] = {{1, deltaT, Math.pow(deltaT,2)/2, Math.pow(deltaT,3)/6, Math.pow(deltaT,4)/24, Math.pow(deltaT,5)/120},
